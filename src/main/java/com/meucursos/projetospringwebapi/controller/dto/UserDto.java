...
public record UserDto(Long id,
    ...
    public User toModel() {
        ...
        model.setAccount(ofNullable(this.account).map(AccountDto::toDto).orElse(null));
        model.setCard(ofNullable(this.card).map(CardDto::toDtoCard).orElse(null));
        model.setFeatures(ofNullable(this.features).orElse(emptyList()).stream().map(FeatureDto::toDtoFeature).collect(toList()));
        model.setNews(ofNullable(this.news).orElse(emptyList()).stream().map(NewsDto::toDtoNews).collect(toList()));
        return model;
    }
